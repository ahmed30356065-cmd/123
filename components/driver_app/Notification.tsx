...
const Notification: React.FC<NotificationProps> = ({ message, type, onClose }) => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setExiting(true);
      const exitTimer = setTimeout(onClose, 300); // Reduced exit animation duration
      return () => clearTimeout(exitTimer);
    }, 2000); // Reduced visibility to 2 seconds
  ...
  return (
    <>
      <style>{`
        ...
        .animate-slide-down { animation: slideDown 0.3s cubic-bezier(0.25, 1, 0.5, 1) forwards; }
