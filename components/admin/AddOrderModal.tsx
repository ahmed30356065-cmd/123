...
import { ChevronLeftIcon, SearchIcon, PlusIcon, MinusIcon, BuildingStorefrontIcon, UserIcon, PhoneIcon, MapPinIcon, ClipboardListIcon, CheckCircleIcon, XIcon, ChevronDownIcon, GridIcon, UtensilsIcon, ShoppingCartIcon } from '../icons';
import useAndroidBack from '../../hooks/useAndroidBack';

interface AddOrderModalProps {
  merchants: User[];
  onClose: () => void;
  onSave: (newOrder: { customer: Customer, notes?: string, merchantId: string, merchantName: string } | { customer: Customer, notes?: string, merchantId: string, merchantName: string }[]) => Promise<void> | void;
}
...
const AddOrderModal: React.FC<AddOrderModalProps> = ({ merchants, onClose, onSave }) => {
  ...
  const handleSave = async () => {
    ...
    try {
        if (orderCount === 1) {
            // Single Add - Clone customer to be safe
            await onSave({ customer: { ...customer }, notes, merchantId, merchantName });
            setProgress(100);
        } else {
            // Bulk Add Optimization
            const ordersPayload = [];
            for (let i = 0; i < orderCount; i++) {
                // Ensure strictly cloned data for each iteration
                ordersPayload.push({ 
                    customer: { ...customer }, 
                    notes, 
                    merchantId, 
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {filteredMerchants.map(m => (
                                <button
                                    ...
                                    className={`w-full flex items-center p-3 rounded-xl border transition-all active:scale-[0.98] ${
                                        merchantId === m.id 
